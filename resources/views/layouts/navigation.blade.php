        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-foreground hover:text-primary transition-colors">
                        {{ __('Dasbor') }}
                    </x-nav-link>
                    <a href="{{ route('products') }}" class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-semibold text-muted-foreground hover:text-primary transition-colors">
                        {{ __('Belanja') }}
                    </a>
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-semibold rounded-md text-muted-foreground bg-card hover:text-foreground focus:outline-none transition ease-in-out duration-150">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')" class="text-foreground hover:bg-muted font-medium">
                            {{ __('Profil Saya') }}
                        </x-dropdown-link>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                    onclick="event.preventDefault();
                                                this.closest('form').submit();" class="text-destructive hover:bg-destructive/10 font-medium">
                                {{ __('Keluar') }}
                            </x-dropdown-link>
                        </form>
                    </x-slot>
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();" class="text-destructive">
                        {{ __('Keluar') }}
                    </x-responsive-nav-link>
                </form>
            </div>
